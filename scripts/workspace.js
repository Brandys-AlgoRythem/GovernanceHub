...
const defaultWorkspace = {
  businessName: 'Northstar Business Group',
  entityType: 'Limited Liability Company',
  state: 'Delaware',
  ownerName: 'Operations Lead',
  primaryConcern: 'Audit readiness',
...
function hydrateWorkspace(workspace = readWorkspace()) {
  ...
  document.querySelectorAll('[data-workspace-status]').forEach((node) => {
    node.textContent = localStorage.getItem(WORKSPACE_KEY) ? 'Custom sample saved' : 'Using starter profile';
  });
}
